...
RSpec.describe Account, :type => :model do
    ...
    describe ".deposit" do
        ...
        context "when called with positive number" do
            it "increases available balance" do
                expect { account.deposit(10000) }.to change { account.available_balance }.by(10000)
            end
        end

        context "when called with negative number" do
            it "prohibits negative deposit" do
                expect(account.deposit(-1000)).to eql [false, "Deposit amount cannot be negative"]
            end
        end

        context "when called with zero" do
            it "prohibits zero dollar deposit" do
                expect(account.deposit(0)).to eql [false, "Deposit amount cannot be zero dollars"]
            end
        end
    ...
    describe ".withdraw" do
        ...
        let(:account) { user.accounts.create(:name => ENV["account_name"], :available_balance => 100000) }

        context "when called with positive number" do
            it "decreases available balance" do
                expect { account.withdraw(-10000) }.to change { account.available_balance }.by(-10000)
            end
        end

        context "when called with negative number" do
            it "prohibits negative withdrawal (addition)" do
                expect(account.withdraw(10000)).to eql [false, "Please format withdrawal amount as negative number"]
            end
        end

        context "when called with zero" do
            it "prohibits zero dollar withdrawal" do
                expect(account.withdraw(0)).to eql [false, "Withdrawal amount cannot be zero dollars"]
            end
        end
