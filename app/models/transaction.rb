class Transaction < ApplicationRecord
    ...
    def update_account(leftover)
        if self.description == "Withdrawal"
            ...
            self.account.update(:asset_balance => new_asset_balance)
    ...
    def update_holding
        if self.description == "Withdrawal" || self.description == "Investment"
            ...
            new_units = self.description == "Withdrawal" ? self.account.holding.units - units_transacted : self.account.holding.units + units_transacted
    ...
    def sufficient_funds
        if self.description == "Withdrawal"
            ...
            errors.add(:amount, "exceeds asset balance") if new_asset_balance < 0
        elsif self.description == "Investment"
            new_available_balance = self.account.available_balance - self.amount
